...
import pyperclip
import pytest
from interactive_build_command import (
    HydraOverride,
...
from retrieval_core.utils.io import write_json
...
def test_extracts_required_defaults_from_stage_and_pipeline_configs() -> None:
    ...
    pipeline_required = extract_required_defaults(
        CONFIG_DIR / "pipeline" / "indexing" / "dense" / "documents_in_memory.yaml"
    )
...
def test_collects_selected_config_graph_with_mount_prefixes() -> None:
    selected = collect_selected_configs(
        ...
        overrides=[
            ...
            HydraOverride("pipeline/inference@pipeline=retrieve/dense_in_memory"),
    ...
    assert [(item.label, item.field_prefix, item.status) for item in selected] == [
        ...
        (
            "component/retriever@pipeline.components.retriever=persisted_in_memory",
...
def test_render_command_preserves_hydra_override_syntax() -> None:
    ...
    assert (
        ...
        "pipeline/inference@pipeline=retrieve/dense_in_memory selections/embedding_model=e5/small_v2 "
        "runtime=gpu"
    )
...
def test_configure_flow_accepts_generated_dense_index_id(tmp_path: Path) -> None:
    ...
    assert result.command == (
        "uv run stage indexing dataset=toy pipeline/indexing@pipeline=dense/documents_in_memory "
...
def test_configure_flow_accepts_generated_input_mapping_run_id(tmp_path: Path) -> None:
    result, output = _run_with_answers(
        ["4", "3", "1", "n", ""],
        include_output=True,
        input_mappings_dir=tmp_path / "input_mappings",
    )

    assert result.command == (
        "uv run stage prepare_mapping dataset=toy input_mapping_recipe=dev_tiny "
        "stage.run_id=toy-dev-tiny"
    )
    assert result.overrides[-1] == "stage.run_id=toy-dev-tiny"
    assert "input mapping run id [toy-dev-tiny]: " in output


def test_configure_flow_can_override_generated_input_mapping_run_id(
    tmp_path: Path,
) -> None:
    result = _run_with_answers(
        ["4", "3", "1", "n", "custom-mapping"],
        input_mappings_dir=tmp_path / "input_mappings",
    )

    assert result.overrides[-1] == "stage.run_id=custom-mapping"


def test_generated_input_mapping_run_id_avoids_existing_directory(
    tmp_path: Path,
) -> None:
    input_mappings_dir = tmp_path / "input_mappings"
    (input_mappings_dir / "toy-dev-tiny").mkdir(parents=True)
    ...
    assert result.overrides[-1] == "stage.run_id=toy-dev-tiny-2"
...
def test_configure_flow_builds_inference_dense_command_with_top_k(
    ...
    assert result.command == (
        "uv run stage inference dataset=toy pipeline/inference@pipeline=retrieve/dense_in_memory "
        "runtime=gpu "
        "selections/embedding_model=e5/small_v2 "
...
def test_configure_flow_builds_evaluation_command_with_metrics(tmp_path: Path) -> None:
    runs_dir = tmp_path / "runs"
    _write_completed_inference_run(runs_dir, "inference_20260101", dataset_name="toy")
    _write_completed_inference_run(runs_dir, "other-dataset", dataset_name="other")

    result, output = _run_with_answers(
        ["1", "3", "n", "1", "Recall@10,MRR@10,NDCG@10"],
        include_output=True,
        runs_dir=runs_dir,
    )
    ...
    assert "  1. inference_20260101" in output
    assert not any("other-dataset" in line for line in output)
...
def test_render_inference_command_accepts_prepared_input_mapping_name() -> None:
    ...
    assert command.endswith("selections.input_mapping=toy_dev")


def test_configure_flow_selects_completed_input_mapping_for_dataset(
    tmp_path: Path,
) -> None:
    input_mappings_dir = tmp_path / "input_mappings"
    mapping_dir = input_mappings_dir / "toy-dev"
    mapping_dir.mkdir(parents=True)
    (mapping_dir / INPUT_MAPPING_FILENAME).write_text("{}\n", encoding="utf-8")
    (mapping_dir / INPUT_MAPPING_METADATA_FILENAME).write_text(
        '{"dataset": "toy"}\n',
        encoding="utf-8",
    )
    ...
    assert result.overrides[-2:] == (
        "selections.input_mapping=toy-dev",
        "selections.index_id=index-1",
    )
    assert "  1. All documents (no input mapping)" in output
    assert "  2. toy-dev" in output
...
def test_clipboard_failure_does_not_fail_command_builder() -> None:
    output: list[str] = []
    ...
    assert output == ["Could not copy command to clipboard: no clipboard backend"]
...
def _run_with_answers(
    ...
    *,
    ...
    input_mappings_dir: Path | None = None,
    ...
    if indexes_dir is not None:
        ...
        (index_dir / "index.json").touch()
        if input_mappings_dir is None:
            input_mappings_dir = indexes_dir.parent / "input_mappings"
    ...
    result = run_configure(
        input_fn=input_fn,
        output_fn=output.append,
        clipboard_copy_fn=copied_commands.append,
        config_dir=CONFIG_DIR,
    ...
    assert copied_commands == [result.command]
    assert "Command copied to clipboard." in output
    if include_output:
        return result, output
    return result


def _write_completed_inference_run(
    runs_dir: Path,
    run_id: str,
    *,
    dataset_name: str,
