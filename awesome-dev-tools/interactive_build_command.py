...
def run_configure(
    *,
    ...
    clipboard_copy_fn: ClipboardCopyFn = pyperclip.copy,
    ...
    output_fn("Command:")
    output_fn(command)
    _copy_command_to_clipboard(
        command,
        copy_fn=clipboard_copy_fn,
        output_fn=output_fn,
    )
    return BuiltCommand(
...
def _copy_command_to_clipboard(
    command: str,
    *,
...
def _prompt_configured_overrides(
    ...
    *,
    ...
    prompt_items = {
        ...
        "inference": [
            {
                "path": "selections.input_mapping",
                "prompt": "input mapping",
                "type": "existing_input_mapping",
            },
            {
                "path": "selections.index_id",
                "prompt": "index id",
        ...
        "prepare_mapping": [
            {
                "path": "stage.run_id",
                "prompt": "input mapping run id",
                "type": "new_input_mapping_id",
                "require_non_empty": True,
            },
        ],
        "evaluation": [
            {
                ...
                "type": "existing_inference_run_id",
                "require_non_empty": True,
            },
            {
                "path": "metrics",
    ...
    for prompt_config in prompt_items:
        ...
        if prompt_type == "existing_input_mapping":
            configured_input_mappings_dir = input_mappings_dir or project_path(
                cfg.paths.input_mappings_dir
            )
            dataset_name = str(_get_config_value(cfg, "dataset.name", default=""))
            available_input_mapping_ids = discover_input_mapping_ids(
                configured_input_mappings_dir,
                dataset_name=dataset_name,
            )
            all_documents_label = "All documents (no input mapping)"
            choices = [
                (None, all_documents_label),
                *(
                    (input_mapping_id, input_mapping_id)
                    for input_mapping_id in available_input_mapping_ids
                ),
            ]
            selected_input_mapping_id, _ = _prompt_menu(
                f"Choose {prompt_text}:",
                choices,
                input_fn=input_fn,
                output_fn=output_fn,
                format_item=lambda item: item[1],
            )
            current_value = _get_config_value(cfg, path, default=None)
            if selected_input_mapping_id is not None:
                _upsert_override(
                    overrides,
                    HydraOverride(compose=f"{path}={selected_input_mapping_id}"),
                )
            elif _has_prompt_default(current_value):
                _upsert_override(overrides, HydraOverride(compose=f"{path}=null"))
            continue

        if prompt_type == "existing_inference_run_id":
            configured_runs_dir = runs_dir or project_path(cfg.paths.runs_dir)
            dataset_name = str(_get_config_value(cfg, "dataset.name", default=""))
            available_run_ids = discover_inference_run_ids(
                configured_runs_dir,
                dataset_name=dataset_name,
            )
            if not available_run_ids:
                raise SystemExit(
                    f"No completed inference runs for dataset {dataset_name!r} found under "
                    f"{configured_runs_dir}. Run inference first."
                )
            selected_run_id = _prompt_menu(
                f"Choose {prompt_text}:",
                available_run_ids,
                input_fn=input_fn,
                output_fn=output_fn,
        ...
        if prompt_type == "new_index_id" and not has_default:
            ...
            has_default = True
        if prompt_type == "new_input_mapping_id" and not has_default:
            configured_input_mappings_dir = input_mappings_dir or project_path(
                cfg.paths.input_mappings_dir
            )
            plain_value = _suggest_input_mapping_id(
                cfg,
                configured_input_mappings_dir,
            )
            has_default = True
        ...
        if not answer:
            if prompt_type in {"new_index_id", "new_input_mapping_id"} and has_default:
        ...
        if prompt_type == "new_input_mapping_id":
            normalized_input_mapping_id = validate_input_mapping_id(answer)
            configured_input_mappings_dir = input_mappings_dir or project_path(
                cfg.paths.input_mappings_dir
            )
            if (configured_input_mappings_dir / normalized_input_mapping_id).exists():
                raise SystemExit(
                    f"Input mapping {normalized_input_mapping_id!r} already exists under "
                    f"{configured_input_mappings_dir}."
                )
            overrides.append(
                HydraOverride(compose=f"{path}={normalized_input_mapping_id}")
            )
            continue
...
def _suggest_input_mapping_id(cfg: Any, input_mappings_dir: Path) -> str:
    dataset_name = _get_config_value(cfg, "dataset.name", default="")
...
def _get_config_value(cfg: Any, path: str, *, default: Any) -> Any:
    ...
    except (KeyError, MissingMandatoryValue, InterpolationToMissingValueError):
        return default
