...
| --- | --- | --- | --- |
| Indexing | `dense/documents_in_memory` | Embed and persist whole documents in a Haystack in-memory store. | `index_id`, `embedding_model` |
| Indexing | `dense/chunks_in_memory` | Split, embed, and persist document chunks in a Haystack in-memory store. | `index_id`, `embedding_model` |
| Inference | `retrieve/dense_in_memory` | Retrieve from either supported persisted in-memory index granularity. | `index_id`, `embedding_model` |
