...
def resolve_inference_mapping(cfg: DictConfig) -> InferenceMapping:
    ...
    if mapping_type == "generated":
        ...
        recipe_hash = input_mapping_recipe_hash(mapping_cfg)
        generated.metadata["mapping_name"] = str(mapping_cfg.get("name", "generated"))
        generated.metadata["recipe_hash"] = recipe_hash
        generated.metadata["recipe"] = input_mapping_generation_params(mapping_cfg)
        generated.metadata["source_paths"] = {
            "documents": project_path(cfg.dataset.documents_path).as_posix(),
            "queries": project_path(cfg.dataset.queries_path).as_posix(),
            "qrels": project_path(cfg.dataset.qrels_path).as_posix(),
        }
...
def materialized_mapping_path(cfg: DictConfig, mapping_cfg: DictConfig) -> Path:
    """Return the dataset-specific JSON path for a generated input-mapping recipe."""

    recipe_hash = input_mapping_recipe_hash(mapping_cfg)
    name = str(mapping_cfg.get("name", "generated"))
    filename = f"{_mapping_file_stem(name)}.{recipe_hash}.json"
    return project_path(cfg.dataset.documents_path).parent / "input_mappings" / filename
...
def write_generated_mapping(
    ...
    *,
    ...
    write_json(mapping_path, generated.mapping)
    write_json(metadata_path, generated.metadata)
