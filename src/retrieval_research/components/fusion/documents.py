"""Fusion components for ranked document lists."""
...
def _sort_and_limit(documents: list[Document], top_k: int | None) -> list[Document]:
    ranked = sorted(documents, key=lambda document: (document.score or 0.0, document.id or ""), reverse=True)
...
class ReciprocalRankFusion:
    ...
    def run(self, **ranked_lists: list[Document]) -> dict[str, list[Document]]:
        ...
        fused = [
            _copy_with_score(document, fused_scores[document_id])
            for document_id, document in documents_by_id.items()
        ]
        return {"documents": _sort_and_limit(fused, self.top_k)}
...
class ScoreFusion:
    ...
    def run(self, **ranked_lists: list[Document]) -> dict[str, list[Document]]:
        ...
        for source_name, weight in self.weights.items():
            ...
            scores = [float(document.score if document.score is not None else self.missing_score) for document in documents]
