...
TOY_EXPERIMENT_DIR = Path(__file__).parents[1] / "experiments" / "query-repetition-e5-small-toy"
...
def test_project_owned_config_choices_are_namespaced() -> None:
    ...
    assert config_files
    assert all(
        path.relative_to(PROJECT_DIR / "configs").parts[2] == "query_repetition"
...
def test_toy_experiment_run_uses_project_component_and_repository_fixture() -> None:
    ...
    assert (
        Path(cfg.dataset.documents_path).resolve()
        == (repository_root / "data" / "processed" / "toy" / "documents.jsonl").resolve()
    )
