...
uv run stage indexing dataset=beir_scifact pipeline/indexing@pipeline=dense/documents_in_memory selections/embedding_model=e5/small_v2 selections.index_id=$indexId runtime=$Runtime stage.run_id=$indexingRun
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

uv run stage inference dataset=beir_scifact pipeline/inference@pipeline=retrieve/dense_in_memory selections/embedding_model=e5/small_v2 selections.index_id=$indexId runtime=$Runtime stage.run_id=$baselineRun
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

uv run stage inference dataset=beir_scifact pipeline/inference@pipeline=query_repetition/dense_query_repetition selections/embedding_model=e5/small_v2 selections.index_id=$indexId runtime=$Runtime stage.run_id=$repeatedRun
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
