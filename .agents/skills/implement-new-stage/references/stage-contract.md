...
| CLI preparation/validation | `packages/retrieval-core/src/retrieval_core/cli.py` |
...
4. Validate missing config and file-backed inputs during `--validate`.
5. Create the output directory once through `StageContext.from_config` during execution.
...
Add only groups the stage needs. Use `???` for required selections. Put user-friendly command-builder fields in `metadata.command_builder.prompt_overrides`.
...
- Validation: missing files fail without creating a run directory.
- Execution: runner writes the expected result and manifest keys.
- Immutability: an existing run directory is rejected.
- Dry run: execution may use temporary files but does not persist artifacts.
...
From a project directory, also run a representative `uv run stage --validate <stage-name> ...` command.
