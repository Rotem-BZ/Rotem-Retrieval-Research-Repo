...
2. Inspect `docs/components.md`, neighboring components, their `__init__.py` exports, tests, and the pipeline/config that will consume the component.
...
6. Implement the component beside the closest category. Use Haystack's `@component` API and explicit output types. Use dynamic sockets only when topology truly requires them.
...
8. Export the public class from the category `__init__.py`; export it from `retrieval_components.components` only when it is part of the package's convenient public surface.
...
11. Update `docs/components.md` when the shared inventory or native-versus-repository decision changes.
